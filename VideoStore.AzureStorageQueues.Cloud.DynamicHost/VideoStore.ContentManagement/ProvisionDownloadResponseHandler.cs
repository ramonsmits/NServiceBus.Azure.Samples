...
namespace VideoStore.ContentManagement
{
    ...
    {
        static ILog Log = LogManager.GetLogger(typeof(ProvisionDownloadResponseHandler));
        ...
        public void Handle(ProvisionDownloadResponse message)
        {
            ...
            Log.DebugFormat("Download for Order # {0} has been provisioned, Publishing Download ready event", message.OrderNumber);
            ...
            Log.DebugFormat("Downloads for Order #{0} is ready, publishing it.", message.OrderNumber);
        }
    }
}
